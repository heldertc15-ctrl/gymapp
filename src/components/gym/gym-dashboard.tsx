...
export function GymDashboard() {
  ...
  async function importBackupFile(file: File | undefined) {
    ...
    try {
      ...
      setBackupStatus(`Imported ${imported.workouts} workouts and ${imported.templates} template exercises`);
    } catch {
          ...
          <Card className="border-lime-300">
            <CardContent className="space-y-5 p-5">
              <div className="text-center">
                ...
                <ExercisePr name={currentExercise.name} workouts={workouts} />
              </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <Card className="border-amber-300">
          <CardHeader>
            ...
            <CardDescription>Your strongest completed set for each exercise.</CardDescription>
          </CardHeader>
          <CardContent>
            {prEntries.length > 0 ? (
              <div className="grid gap-2">
                {prEntries.slice(0, 8).map(([name, pr]) => (
                  <div key={name} className="flex items-center justify-between gap-3 rounded-lg border border-zinc-100 bg-zinc-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-900">
                    <div className="min-w-0">
                      <div className="truncate font-black capitalize text-zinc-950 dark:text-zinc-50">{name}</div>
                      <div className="text-xs font-semibold text-zinc-500 dark:text-zinc-400">{formatVolume(pr.value)} best set volume</div>
                    </div>
              ...
              <EmptyState title="No PRs yet" body="Complete a set with weight and reps to create your first PR." />
            )}
          </CardContent>
        </Card>
      </section>
...
function ExercisePr({ name, workouts }: { name: string; workouts: Workout[] }) {
  const pr = getExercisePr(name, workouts);
