...
export const GYM_STORAGE_KEYS = {
  ...
  manualPrs: "momentum.gym.manualPrs",
};
...
export function calculateGymStats(workouts: Workout[], manualPrs: ManualPr[] = [], now = new Date()): GymStats {
  ...
  manualPrs.forEach((manualPr) => {
    const name = normalizeExerciseName(manualPr.name);
    const weight = Number(manualPr.weight);
    const reps = Number(manualPr.reps);

    if (!name || !Number.isFinite(weight) || !Number.isFinite(reps)) {
      return;
    }
    ...
    if (!prs[name] || value >= prs[name].value) {
      prs[name] = { weight: manualPr.weight, reps: manualPr.reps, value };
    }
  });
...
export function getExercisePr(exerciseName: string, workouts: Workout[], manualPrs: ManualPr[] = []) {
  const stats = calculateGymStats(workouts, manualPrs).prs;
...
export function sanitizeManualPrs(value: unknown): ManualPr[] {
  if (!Array.isArray(value)) {
    return [];
  }

  return value.filter(isManualPr);
}
...
function isManualPr(value: unknown): value is ManualPr {
  if (!value || typeof value !== "object") {
    return false;
  }

  const pr = value as Partial<ManualPr>;

  return (
    typeof pr.id === "string" &&
    typeof pr.name === "string" &&
    typeof pr.weight === "string" &&
    typeof pr.reps === "string" &&
